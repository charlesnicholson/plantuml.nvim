...
TEMP_DIR=$(mktemp -d)
LOG_FILE="$TEMP_DIR/plugin-loading.log"
...
if nvim --headless -u ~/.config/nvim/init.lua -c "lua local p = require('plantuml'); p.setup({auto_start = false, http_port = 8764}); print('Plugin loaded successfully')" -c "qall!" 2>&1 | tee -a "$LOG_FILE"; then
...
for func in $EXPECTED_FUNCTIONS; do
    if nvim --headless -u ~/.config/nvim/init.lua -c "lua local p = require('plantuml'); p.setup({auto_start = false}); assert(type(p.$func) == 'function', '$func not found'); print('Function $func found')" -c "qall!" 2>&1 | tee -a "$LOG_FILE"; then
...
if nvim --headless -u ~/.config/nvim/init.lua -c "lua local p = require('plantuml'); p.setup({auto_start = false}); p.start(); print('Server started'); vim.wait(1000); p.stop(); print('Server stopped')" -c "qall!" 2>&1 | tee -a "$LOG_FILE"; then
...
if nvim --headless -u ~/.config/nvim/init.lua -c "lua local p = require('plantuml'); p.setup({auto_start = false, http_port = 8764}); local config = p.get_config(); assert(config.http_port == 8764, 'Wrong port'); print('Configuration loaded correctly')" -c "qall!" 2>&1 | tee -a "$LOG_FILE"; then
