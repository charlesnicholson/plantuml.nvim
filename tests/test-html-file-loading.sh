...
if [ -f "$HTML_FILE" ]; then
    echo "✓ HTML file exists at $HTML_FILE"
else
...
echo "Test 5: Verify HTML file contains JavaScript"
...
for js_function in $JS_FUNCTIONS; do
    if grep -q "function $js_function" "$HTML_FILE"; then
        echo "✓ JavaScript function '$js_function' found in HTML file"
    else
        echo "✗ JavaScript function '$js_function' not found in HTML file"
        exit 1
    fi
done

# Test 6: Test HTML file loading in Neovim
echo "Test 6: Test HTML file loading in Neovim"
...
echo "Neovim output: $NVIM_OUTPUT"

if echo "$NVIM_OUTPUT" | grep -q "HTML_LOADING_SUCCESS"; then
    echo "✓ HTML file loads successfully in Neovim"
elif echo "$NVIM_OUTPUT" | grep -q "HTML_LOADING_ERROR"; then
    echo "✗ HTML file loading failed in Neovim"
    echo "$NVIM_OUTPUT"
    exit 1
else
    echo "✗ Unexpected output from Neovim test"
    echo "$NVIM_OUTPUT"
    exit 1
fi
